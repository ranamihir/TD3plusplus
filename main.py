...
def evaluate_policy(policy, filter_obs,eval_episodes=10):
    avg_reward = 0.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tau", default=0.01, type=float)  # Target network update rate
    parser.add_argument(
    ...
    parser.add_argument(
        "--norm_obs", action='store_true',
        default=False)  # Normalize observations
    parser.add_argument(
        "--norm_ret", action='store_true', default=False)  # Normalize returns
    parser.add_argument(
        "--prefix", default="")  # Log prefix
    args = parser.parse_args()

    file_name = "%s%s_%s_%s" % (args.prefix, args.policy_name, args.env_name, str(args.seed))
    print("---------------------------------------")
    ...
    replay_buffer = utils.ReplayBuffer(
        ...
        norm_ret=args.norm_ret,
        discount=args.discount,
        obs_shape=(state_dim, ))

    # Evaluate untrained policy
    evaluations = [evaluate_policy(policy, replay_buffer.filter_obs)]
    ...
    while total_timesteps < args.max_timesteps:
        ...
        if done:
            ...
            if timesteps_since_eval >= args.eval_freq:
                ...
                evaluations.append(evaluate_policy(policy, replay_buffer.filter_obs))
        ...
        if total_timesteps < args.start_timesteps:
            ...
            obs_ = replay_buffer.filter_obs(obs)
            action = policy.select_action(np.array(obs_))
